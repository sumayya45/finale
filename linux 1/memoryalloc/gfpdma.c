...
static int __init my_module_init(void) { 
    ...
    buffer_dma = kmalloc(1024, GFP_DMA); 

    if (!buffer_dma) { 

        printk(KERN_ALERT "my_module: kmalloc GFP_DMA failed!\n"); 

        return -ENOMEM; 

    } 

    printk(KERN_INFO "my_module: kmalloc GFP_DMA succeeded\n"); 
...
static void __exit my_module_exit(void) { 
    ...
    if (buffer_dma) { 
        ...
        printk(KERN_INFO "my_module: kfree buffer_dma succeeded\n"); 
